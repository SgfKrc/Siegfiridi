...
def test_edit_commands_round_trip_through_undo_redo() -> None:
    ...
    assert stack.undo() and project.tracks[0].notes[0].velocity == 100
    assert stack.undo() and project.tracks[0].notes[0].duration_tick == 480
    assert stack.undo() and project.tracks[0].notes[0].start_tick == 0
    assert stack.undo() and len(project.tracks[0].notes) == 1
    assert stack.redo() and len(project.tracks[0].notes) == 2
